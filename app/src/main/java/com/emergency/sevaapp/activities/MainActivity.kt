...
//        requestNotificationPermission()
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channelName = "Weekly Notifications"
            val channelDescription = "Notifications for weekly reminders"
            val importance = NotificationManager.IMPORTANCE_HIGH

            val channel = NotificationChannel("weekly_notifications", channelName, importance).apply {
    ...
    private fun scheduleWeeklyNotification() {
        ...
        val pendingIntent = PendingIntent.getBroadcast(
            ...
            0,
            alarmIntent,
            PendingIntent.FLAG_IMMUTABLE  // Set FLAG_IMMUTABLE here
        )

        val alarmManager = getSystemService(Context.ALARM_SERVICE) as AlarmManager

        // Schedule the notification to trigger every 7 days (1 week)
//        val intervalMillis =  10 * 100L  // 7 days in milliseconds
        val intervalMillis = 3 * 24 * 60 * 60 * 1000L  // 3 days in milliseconds

        alarmManager.setRepeating(
            AlarmManager.RTC_WAKEUP,
            System.currentTimeMillis() + intervalMillis,  // Start the alarm after 1 week
            intervalMillis,  // Repeat every week
            pendingIntent
        )
    }
    ...
    private fun onHeartDrawableClick() {
        ...
        val locationAddressCurrent =
            dialogView.findViewById<TextView>(R.id.location_address_current)
...
//          Access navigation header
    ...
    private fun showGifProgressDialog1(): AlertDialog {
        ...
        val gifUrl =
            "https://emergencyseva.in/emergencyseva.in/images/KAdwRRUAsQ-ezgif.com-crop.gif"

        // Validate the URL before loading
        if (gifUrl.isNotEmpty() && isValidUrl(gifUrl)) {
            try {
                Glide.with(this)
                    .asGif()
                    .load(gifUrl)
                    .into(imageView)
//                    .onLoadFailed(ContextCompat.getDrawable(this, R.drawable.running_ambulance)) // Handle failure gracefully
            } catch (e: Exception) {
                e.printStackTrace()
                Log.e("GlideError", "Error loading GIF: ${e.message}")
            }
        } else {
            Log.e("Error", "Invalid or empty URL for Glide: $gifUrl")
        }
        ...
        // **Use CountDownTimer to strictly enforce 5 seconds**
        object : CountDownTimer(5000, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                // No action needed here, just let the timer run
            }
...
//        val gifUrl = "https://emergencyseva.in/emergencyseva.in/images/KAdwRRUAsQ-ezgif.com-crop.gif"
        val gifUrl =
            "https://i.giphy.com/media/v1.Y2lkPTc5MGI3NjExdWJmNzJ0MmNpMGw0djhzZXZidG9mcXJ3cW5mdmRtdzd6ajhxbjBqaCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/wSnOQErP2oRnk4RB36/giphy.gif"
